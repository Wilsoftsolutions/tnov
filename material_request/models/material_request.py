...
class MaterialRequest(models.Model):
	...
	def create_mo_order(self):
		...
		location_id = self.env['stock.warehouse'].search([('company_id', '=', self.env.company.id)],limit=1).lot_stock_id
		for rec in self:
	...
	requested_by = fields.Many2one('res.users', 'Requester', required=True,default=_get_default_requester)
	...
	line_ids = fields.One2many('material.request.line', 'material_id','Products to Material', readonly=False,copy=True,)
